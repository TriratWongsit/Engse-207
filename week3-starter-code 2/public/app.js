...
async function createTask(taskData) {
    ...
    try {
        ...
        // Construct full task object based on response
        const newTask = {
            id: data.id,
...
function renderTasks() {
    ...
    // Assuming filter is for Priority based on typical labs, or match exact requirement if Status
    // If the select box has values "HIGH", "MEDIUM", "LOW":
    if (currentFilter !== 'ALL') {
        filteredTasks = allTasks.filter(task => task.priority === currentFilter);
    }
